...
// 抄的
vector<int> recoverArray(vector<int> const& A)
{
	vector<int> R;
	map<int, int> M;
	for (int i : A)
	...
	{
		int k = (it->first - start) / 2;
		if (k <= 0 || start + 2 * k != it->first)
			continue;
		auto m = M;
		while (!m.empty())
		{
			auto lo = m.begin();
			auto hi = m.find(lo->first + 2 * k);
			if (hi == m.end())
				break;
			for (int i = lo->second; i--;)
				R.push_back(lo->first + k);
			hi->second -= lo->second;
			if (hi->second < 0)
				break;
			if (hi->second == 0)
...
int main()
{
	ToOut(recoverArray({2, 10, 6, 4, 8, 12}));
	ToOut(recoverArray({1, 1, 3, 3}));
	ToOut(recoverArray({5, 435}));
}
