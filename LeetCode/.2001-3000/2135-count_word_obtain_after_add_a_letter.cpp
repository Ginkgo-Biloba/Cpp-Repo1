...
给你两个下标从 0 开始的字符串数组 startWords 和 targetWords 。
每个字符串都仅由 小写英文字母 组成。
...
转换操作 如下面两步所述：
	...
	例如，"abcd" 可以重排为 "acbd"、"bacd"、"cbda"，以此类推。
	注意，它也可以重排为 "abcd" 自身。
...
int wordCount(vector<string> const& start, vector<string> const& target)
{
	unordered_set<unsigned> S;
	for (string const& w : start)
	{
		unsigned m = 0;
		for (int i : w)
			m |= 1u << (i - 'a');
