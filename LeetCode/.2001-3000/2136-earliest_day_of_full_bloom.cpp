...
给你两个下标从 0 开始的整数数组 plantTime 和 growTime ，每个数组的长度都是 n ：
	...
	plantTime[i] 是 播种 第 i 枚种子所需的 完整天数 。
	每天，你只能为播种某一枚种子而劳作。
...
int earliestFullBloom(
	vector<int> const& plant, vector<int> const& grow)
{
	int n = static_cast<int>(plant.size());
	vector<int> index;
	for (int i = 0; i < n; ++i)
		index.push_back(i);
	sort(index.begin(), index.end(),
		[&grow](int a, int b) -> bool
		{ return grow[a] > grow[b]; });
	int ans = 0, last = 0;
	for (int i : index)
	{
		last += plant[i];
		ans = max(ans, last + grow[i]);
	}
	return ans;
}
