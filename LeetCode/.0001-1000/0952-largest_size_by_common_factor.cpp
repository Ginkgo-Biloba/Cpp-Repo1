...
public:
	...
	int largestComponentSize(vector<int>& A)
	{
		...
		genPrime(static_cast<int>(::sqrt(N)) + 1);
		par.resize(N);
		num.resize(N);
		for (int i = 0; i < N; ++i)
		{
		...
		for (int a : A)
		{
			// for (int p : prime) // 为啥这个不对？
			int p = static_cast<int>(::sqrt(a));
			for (; p >= 2; --p)
			{
				int k = a / p;
				// if (k < p) break;
