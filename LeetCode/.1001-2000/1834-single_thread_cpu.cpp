...
{
	using elem = array<int, 3>;
	int len = static_cast<int>(task.size());
	vector<elem> T;
	for (int i = 0; i < len; ++i)
		T.push_back({ task[i][0], task[i][1], i });
	sort(T.begin(), T.end(),
	...
	{
		if (Q.empty())
			idle = max<int64_t>(idle, T[i][0]);
		for (; (i < len) && (T[i][0] <= idle); ++i)
			Q.push({ -T[i][1], -T[i][2], T[i][0] });
		elem e = Q.top();
		R.push_back(-e[1]);
		idle -= e[0];
		Q.pop();
...
int main()
{
	vector<vector<int>>
		a = { { 1, 2 }, { 2, 4 }, { 3, 2 }, { 4, 1 } },
		b = { { 7, 10 }, { 7, 12 }, { 7, 5 }, { 7, 4 }, { 7, 2 } },
		c = { { 19, 13 }, { 16, 9 }, { 21, 10 }, { 32, 25 }, { 37, 4 }, { 49, 24 }, { 2, 15 }, { 38, 41 }, { 37, 34 }, { 33, 6 }, { 45, 4 }, { 18, 18 }, { 46, 39 }, { 12, 24 } };
	ToOut(getOrder(a));
	ToOut(getOrder(b));
	ToOut(getOrder(c));
}
