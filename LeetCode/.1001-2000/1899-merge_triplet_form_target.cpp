...
{
	int len = static_cast<int>(triplets.size());
	int dst[3] = {0, 0, 0};
	for (int i = 0; i < len; ++i)
	{
		int* ptr = triplets[i].data();
		if (ptr[0] > target[0] || ptr[1] > target[1] || ptr[2] > target[2])
			continue;
		if (ptr[0] < target[0] && ptr[1] < target[1] && ptr[2] < target[2])
			continue;
		dst[0] += ptr[0] == target[0];
		dst[1] += ptr[1] == target[1];
		dst[2] += ptr[2] == target[2];
	}
