...
bool areSentencesSimilar(string a, string b)
{
	...
	int s = -1, l = alen, d = blen - alen;
	for (int i = 0; i < alen && a[i] == b[i]; ++i)
	{
		if (a[i] == ' ' || b[i + 1] == ' ')
			s = i;
	}
	for (int i = alen - 1; i >= max(0, s) && a[i] == b[i + d]; --i)
	{
		if (a[i] == ' ' || b[i + d - 1] == ' ')
			l = i;
	}
	return s == alen - 1 || l == 0 || s == l;
}


int main()
{
	ToOut(areSentencesSimilar("My name is Haley", "My Haley"));
	ToOut(areSentencesSimilar("of", "A lot of words"));
	ToOut(areSentencesSimilar("Eating right now", "Eating"));
	ToOut(areSentencesSimilar("Luky", "Lucccky"));
}
