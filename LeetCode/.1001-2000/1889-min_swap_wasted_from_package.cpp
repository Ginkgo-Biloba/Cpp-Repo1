...
如果 无法 将所有包裹放入箱子中，请你返回 -1 。由于答案可能会 很大 ，请返回它对 10^9 + 7 取余 的结果。
...
int minWastedSpace(vector<int>& pack, vector<vector<int>>& boxes)
{
	...
	for (vector<int>& box : boxes)
	{
		...
		if (box.back() < pack.back())
			continue;
		for (int b : box)
		{
			auto ik = upper_bound(it, pack.end(), b);
			sum += static_cast<int64_t>(ik - it) * b;
			it = ik;
		}
		use = min(use, sum);
	}
	if (use == INT64_MAX)
		return -1;
	use -= accumulate(pack.begin(), pack.end(), INT64_C(0));
	use %= static_cast<int64_t>(1e9 + 7);
	return static_cast<int>(use);
}
